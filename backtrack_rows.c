...
typedef struct _square{
  ...
  // for the mth row
  int* pairs; // an array of bits to keep track of the availability of adjacent
...
void set_cell(square square, int row, int col, char symbol) { // TODO: make apis consistent
  ...
  // adjacent pairs
  if (col > 0) {
	set_avail(square.pairs, square.grid[index - 1], square.grid[index], false);
	set_avail(square.pairs, square.grid[index - 1], symbol, true);
  }
...
bool any_possibilities(square square, int row, int col) {
  ...
  int prev_index = ((square.size * row) + col) - 1;
  int used_union = (mask & square.row_used[row]) |
	(mask & square.col_used[col]) |
	(mask & square.pairs[square.grid[prev_index]]);
...
bool is_avail(square square, char symbol, int row, int col) {
  int mask = 1 << symbol;
  int prev_index = ((square.size * row) + col) - 1;
  if ((square.row_used[row] & mask) ||
	  (square.col_used[col] & mask) ||
	  (square.pairs[square.grid[prev_index]] & mask)) {
	return false;
  }
  return true;
}
...
square new_square(int size) {
  ...
  square.pairs = malloc(size * sizeof(int));
